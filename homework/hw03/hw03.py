...
def balanced(m):
    ...
    if is_planet(m):
        return True
    assert is_mobile(m), 'only a planet or a mobile can be judged about balance'
    lm, rm = left(m), right(m)
    left_torque = length(lm) * total_weight(end(lm))
    right_torque = length(rm) * total_weight(end(rm))
    return left_torque == right_torque and balanced(end(lm)) and balanced(end(rm))
...
def replace_leaf(t, find_value, replace_value):
    ...
    my_branches = [replace_leaf(b, find_value, replace_value) for b in branches(t)]
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    f = lambda t: a*t*t + b*t + c # use lambda to define simple function
    left, right = lower_bound(x), upper_bound(x)
    fl, fr = f(left), f(right)
    if a == 0:
        return interval(fl,fr) if b >= 0 else interval(fr,fl)
    mid = -b / (2*a)
