...
def missing_digits(n):
    ...
    if n < 10:
        ...
        if difference > 1:
            return missing_digits(n // 10) + difference - 1
        else:
...
def count_coins(total):
    ...
    True
    """
    def count_coins_helper(remain, min_value):
        if remain < 0 or min_value == None:
            return 0
        if remain == 0:
            return 1
        # sort the situation into two kinds:
        # with or without smallest coins
        with_smallest = count_coins_helper(remain - min_value, min_value)
        without_smallest = count_coins_helper(remain, next_largest_coin(min_value))
