class VendingMachine:
    ...
    def vend(self):
        if self.amount == 0:
            ...
            self.funds, self.amount = 0, self.amount-1
            return 'Here is your {0}.'.format(self.product)
        else:
            changes, self.funds, self.amount = self.funds - self.price, 0, self.amount-1
            return 'Here is your {0} and ${1} change.'.format(self.product, changes)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        d = Mint.current_year - self.year - 50
        d = d if d >= 0 else 0
        return d + self.cents
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        b = t.branches[0]
        return is_bst(b) and (bst_min(b) > t.label or bst_max(b) <= t.label)
    else:
