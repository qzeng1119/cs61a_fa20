...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if not self.is_buffed:
            self.damage *= 2
            self.is_buffed = True
        if self.is_container() and self.contained_ant:
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    min_range = 0
    max_range = float('inf') # returns an infinite positive value represented as a float that can be compared with other numbers
    ...
    def nearest_bee(self, beehive):
        ...
        while cur is not beehive:
            if cur.bees and self.min_range <= i <= self.max_range: # min_range -> Wrong!
                return bee_selector(cur.bees)                      # self.min_range -> Right！
            cur, i = cur.entrance, i + 1                           # Don't forget to use 'self.' when referring to an instance's attribute
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        damage = self.health + self.damage if self.health <= amount else amount
        copy = self.place.bees[:]
        for b in copy:
            b.reduce_health(damage)
        Ant.reduce_health(self, amount) # wrong version: self.reduce_health(amount)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        ...
        super().__init__(health)
    def action(self, gamestate):
        if self.chewing:    # when referencing class or instance attribute, don't forget 'self.'!
            self.chewing -= 1
        else:
            if self.place.bees:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        # Hint: Avoid iterating and mutating a list at the same time, otherwise the iteration might skip some elements.
        copy = self.place.bees[:] # shallow copy
        for b in copy:
            b.reduce_health(self.damage)
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.is_true:
            ...
            super().action(gamestate) # Don't forget this! A real QueenAnt also does what a ThrowerAnt does.
            cur = self.place.exit
            while cur:
                if cur.ant:
                    cur.ant.buff()
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.is_true and self.health <= amount:
            bees_win()
    ...
    # BEGIN Problem EC
    def remove_from(self, place):
        """A real QueenAnt can not be removed."""
        if self.is_true:
            pass
        else:
...
class Bee(Insect):
    ...
    def slow(self, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        # first_version_code: (works without the function 'apply_status')
        # previous_action = self.action
        # def replace(gamestate):
        #     nonlocal length
        #     if length > 0:
    ...
    def scare(self, length):
        ...
        def status(gamestate, previous_action):
            self.is_scared = True
            previous_action(gamestate) # I think it should be this: Bee.action(self, gamestate)
            # Each ant has a status-chain: self.action -> previous_action -> previous_action -> ... -> self_action(without status)
            # The 'previous_action(gamestate)' ensures that each status on the 'buff-chain' only lasts LENGTH turns, because each status on the status-chain is iterated over on each turn
            # The 'Bee.action(self, gamestate)' ensures the positive correlation between cost and effect when a SLOW status and a SCARE status are set on a bee at the same time 
            self.is_scared = False
        self.apply_status(status, self.action, length)
        # END Problem Optional 2
    ...
    def apply_status(self, status, previous_action, length):
        ...
        def new_action(gamestate):
            nonlocal status_count  # This is very important!!!
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 3
        distance, dic, cur = 0, {}, self.place
        while cur is not beehive:
            if cur.ant and cur.ant is not self:
                dic[cur.ant] = distance
            if cur.bees:
                for b in cur.bees:
                    dic[b] = distance
            distance, cur = distance + 1, cur.entrance
        return dic
        # END Problem Optional 3
