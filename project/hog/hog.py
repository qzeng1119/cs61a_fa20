...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    factor = 10
    while factor <= min(player_score, opponent_score):
...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    minus = opponent_score - player_score
    if minus > 0 and minus < 3:
        return True
    else:
        return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_highest(score0, score1):
        cur_score = score0 if who == 0 else score1 # using this can avoid repetition of code effectively
        minus = cur_score - last_score
        if minus > running_high:
            print(minus, "point(s)!", "The most yet for Player", who)
        return announce_highest(who, cur_score, max(minus, running_high))
    return say_highest
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_score = 0
    while cur_num <= 10:
        points = average_dice(cur_num, dice)
        if points > max_score:
            num = cur_num
            max_score = points
        cur_num += 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score + free_score, opponent_score) or free_score >= cutoff:
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    if my score is 1 less than the score we need to get an extra turn, then 
    try a large number, we can get an extra turn(if 1 appears) or acquire 
    lots of points(if all of the dice are not 1) through this way
    else, just use the extra_turn_strategy
    """
    # BEGIN PROBLEM 12
